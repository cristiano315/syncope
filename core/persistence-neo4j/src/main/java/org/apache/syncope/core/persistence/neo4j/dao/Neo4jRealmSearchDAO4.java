...
public class Neo4jRealmSearchDAO4 extends AbstractRealmSearchDAO {

    protected static final String MATCH_NODE = "MATCH (n) ";

    protected static final String MATCH_NODE_WITH_LABEL = "MATCH (n:";
    ...
    protected static void queryOp(
        ...
        query.append("WHERE EXISTS { ").
                append(Strings.CS.prependIfMissing(leftInfo.query().toString(), MATCH_NODE)).
                append(" } ").
                append(op).append(" EXISTS { ").
                append(Strings.CS.prependIfMissing(rightInfo.query().toString(), MATCH_NODE)).
                append(" }");
    }
    ...
    public Optional<Realm> findByFullPath(final String fullPath) {
        ...
        return neo4jClient.query(
                MATCH_NODE_WITH_LABEL + Neo4jRealm.NODE + ") WHERE n.fullPath = $fullPath RETURN n.id").
    ...
    public List<Realm> findByName(final String name) {
        return toList(neo4jClient.query(
                MATCH_NODE_WITH_LABEL + Neo4jRealm.NODE + ") WHERE n.name = $name RETURN n.id").
    ...
    public List<Realm> findChildren(final Realm realm) {
        return toList(neo4jClient.query(
                MATCH_NODE_WITH_LABEL + Neo4jRealm.NODE + " {id: $id})<-[r:" + Neo4jRealm.PARENT_REL + "]-(c) RETURN c.id").
    ...
    public List<Realm> findDescendants(final String base, final String prefix) {
        ...
        StringBuilder query = new StringBuilder(MATCH_NODE_WITH_LABEL).append(Neo4jRealm.NODE).append(") ").
    ...
    protected void wrapQuery(
        ...
        TextStringBuilder match = new TextStringBuilder(MATCH_NODE_WITH_LABEL).append(Neo4jRealm.NODE).append(") ").
    ...
    protected AttrCondQuery getQuery(
        ...
        TextStringBuilder query = new TextStringBuilder(MATCH_NODE);
        switch (cond.getType()) {
            case ISNOTNULL ->
    ...
    protected record AttrCondValue(String value, boolean isStr, boolean lower) {
    ...
    protected void fillAttrQuery(
        ...
        if (handleNotAttrCond(query, attrValue, schema, cond, not, parameters)) {
            return;
        }

        AttrCondValue attrCondValue = attrCondValue(attrValue, schema, cond);
        query.append(WHERE);
        appendAttrCond(query, schema, cond, attrCondValue);
        // shouldn't occour: processed before
    }

    protected boolean handleNotAttrCond(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final PlainSchema schema,
            final AttrCond cond,
        ...
        } else if (schema.isUniqueConstraint()) {
    ...
    protected AttrCondValue attrCondValue(
            final PlainAttrValue attrValue,
            final PlainSchema schema,
            final AttrCond cond) {
        ...
        boolean isStr = schema.getType().isStringClass() || schema.getType() == AttrSchemaType.Date
                || !isParsableNonStringValue(schema.getType(), value);

        return new AttrCondValue(value, isStr, lower);
    }

    protected boolean isParsableNonStringValue(final AttrSchemaType type, final String value) {
        try {
            switch (type) {
    ...
    protected void appendAttrCond(
            final TextStringBuilder query,
            final PlainSchema schema,
            final AttrCond cond,
            final AttrCondValue attrCondValue) {

        switch (cond.getType()) {
            case ISNULL ->
                query.append(StringUtils.EMPTY);

            case ISNOTNULL ->
                query.append(schema.getKey()).append(" IS NOT NULL");

            case ILIKE, LIKE ->
                appendAttrLikeCond(query, schema, attrCondValue);

            case IEQ, EQ ->
                appendAttrEqCond(query, schema, attrCondValue);

            case GE ->
                appendPlainAttrCond(
                        query,
                        schema,
                        " >= " + escapeIfString(attrCondValue.value(), attrCondValue.isStr()));
    ...
    protected void appendAttrLikeCond(
            final TextStringBuilder query,
            final PlainSchema schema,
            final AttrCondValue attrCondValue) {
        ...
        if (schema.getType().isStringClass()) {
            appendPlainAttrCond(
                    query,
                    schema,
                    " =~ \"" + (attrCondValue.lower() ? "(?i)" : "")
                    + AnyRepoExt.escapeForLikeRegex(attrCondValue.value()).replace("%", ".*") + '"');
        } else {
    ...
    protected void appendAttrEqCond(
            final TextStringBuilder query,
            final PlainSchema schema,
            final AttrCondValue attrCondValue) {

        if (StringUtils.containsAny(attrCondValue.value(), AnyRepoExt.REGEX_CHARS) || attrCondValue.lower()) {
            appendPlainAttrCond(
                    query,
                    schema,
                    " =~ \"^" + (attrCondValue.lower() ? "(?i)" : "")
    ...
    protected void fillAttrQuery(
        ...
        if (handleNotAnyCond(query, attrValue, schema, cond, not, parameters)) {
            return;
        }
        ...
        appendAnyCond(query, attrValue, schema, cond, parameters, anyCondProperty(cond, lower), lower);
    }

    protected boolean handleNotAnyCond(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final PlainSchema schema,
            final AnyCond cond,
    ...
    protected String anyCondProperty(final AnyCond cond, final boolean lower) {
    ...
    protected void appendAnyCond(
        ...
        switch (cond.getType()) {
            ...
            case ILIKE, LIKE ->
                appendAnyLikeCond(query, schema, cond, parameters, property, lower);

            case IEQ, EQ ->
                appendAnyEqCond(query, attrValue, parameters, property, lower);

            case GE ->
                appendAnyComparisonCond(query, attrValue, parameters, property, ">=");
    ...
    protected void appendAnyLikeCond(
            final TextStringBuilder query,
            final PlainSchema schema,
            final AnyCond cond,
            final Map<String, Object> parameters,
    ...
    protected void appendAnyEqCond(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final Map<String, Object> parameters,
            final String property,
    ...
    protected void appendAnyComparisonCond(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final Map<String, Object> parameters,
            final String property,
        ...
        query.append(property).append(operator).append('$').append(setParameter(parameters, attrValue.getValue()));
    }
    ...
    protected List<String> parseOrderBy(final Streamable<Sort.Order> orderBy) {
        ...
        orderBy.forEach(clause -> appendOrderByClause(
                clause,
                clauses,
    ...
    protected void appendOrderByClause(
            ...
            final Set<String> orderByUniquePlainSchemas,
            final Set<String> orderByNonUniquePlainSchemas) {

        if (realmUtils.getField(clause.getProperty()).isPresent()) {
            clauses.add(clause.getProperty() + ' ' + clause.getDirection().name());
        } else {
            plainSchemaDAO.findById(clause.getProperty()).ifPresent(schema -> appendPlainSchemaOrderByClause(
                    schema,
                    clause,
                    clauses,
                    orderByUniquePlainSchemas,
    ...
    protected void appendPlainSchemaOrderByClause(
            final PlainSchema schema,
            final Sort.Order clause,
            final List<String> clauses,
            final Set<String> orderByUniquePlainSchemas,
        ...
        validatePlainSchemaOrderBy(orderByUniquePlainSchemas, orderByNonUniquePlainSchemas);
        clauses.add(schema.getKey() + ' ' + clause.getDirection().name());
    }

    protected void validatePlainSchemaOrderBy(
            final Set<String> orderByUniquePlainSchemas,
            final Set<String> orderByNonUniquePlainSchemas) {
