...
public class Neo4jRealmSearchDAO3 extends AbstractRealmSearchDAO {
    ...
    protected record DAODependencies(
            RealmDAO realmDAO,
    ...
    public Neo4jRealmSearchDAO3(final DAODependencies deps) {

        super(deps.plainSchemaDAO(), deps.userDAO(), deps.groupDAO(),
                deps.entityFactory(), deps.validator());
        this.realmDAO = deps.realmDAO();
        this.realmUtils = deps.realmUtils();
        this.neo4jTemplate = deps.neo4jTemplate();
        this.neo4jClient = deps.neo4jClient();
    }

    public Neo4jRealmSearchDAO3(
        ...
        this(new DAODependencies(realmDAO, plainSchemaDAO, userDAO, groupDAO,
                entityFactory, validator, realmUtils, neo4jTemplate, neo4jClient));
    }
    ...
    protected void fillAttrQuery(
        ...
        boolean isStr = true;
        final boolean lower = schema.getType().isStringClass()
                && (cond.getType() == AttrCond.Type.IEQ
                || cond.getType() == AttrCond.Type.ILIKE);
        if (!schema.getType().isStringClass() && schema.getType() != AttrSchemaType.Date) {
            try {
                ...
                LOG.debug("Unable to parse attribute value", nfe);
            }
        }
