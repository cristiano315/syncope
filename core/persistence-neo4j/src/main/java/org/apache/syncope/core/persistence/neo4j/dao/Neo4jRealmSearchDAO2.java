...
public class Neo4jRealmSearchDAO2 extends AbstractRealmSearchDAO {

    private static final String MATCH_N = "MATCH (n) ";

    private static final String MATCH_N_LABEL = "MATCH (n:";
    ...
    public Optional<Realm> findByFullPath(final String fullPath) {
        ...
        return neo4jClient.query(
                MATCH_N_LABEL + Neo4jRealm.NODE + ") WHERE n.fullPath = $fullPath RETURN n.id").
    ...
    public List<Realm> findByName(final String name) {
        return toList(neo4jClient.query(
                MATCH_N_LABEL + Neo4jRealm.NODE + ") WHERE n.name = $name RETURN n.id").
    ...
    public List<Realm> findChildren(final Realm realm) {
        return toList(neo4jClient.query(
                MATCH_N_LABEL + Neo4jRealm.NODE + " {id: $id})<-[r:" + Neo4jRealm.PARENT_REL + "]-(c) RETURN c.id").
    ...
    public List<Realm> findDescendants(final String base, final String prefix) {
        ...
        StringBuilder query = new StringBuilder(MATCH_N_LABEL).append(Neo4jRealm.NODE).append(") ").
    ...
    protected QueryInfo getQuery(final SearchCond cond, final Map<String, Object> parameters) {
        ...
        switch (cond.getType()) {
            ...
            default -> query.append(StringUtils.EMPTY);
        }
    ...
    protected void wrapQuery(
        ...
        TextStringBuilder match = new TextStringBuilder(MATCH_N_LABEL).append(Neo4jRealm.NODE).append(") ").
    ...
    protected void fillAttrQuery(
        ...
        if (not) {
            appendNegatedAttrQuery(query, attrValue, schema, cond, parameters);
            return;
        }

        appendAttrQuery(query, attrValue, schema, cond, parameters);
    }

    private void appendNegatedAttrQuery(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final PlainSchema schema,
            final AttrCond cond,
    ...
    private void appendAttrQuery(
        ...
        switch (cond.getType()) {
            case ISNULL -> query.append(property(schema)).append(" IS NULL");

            case ISNOTNULL -> query.append(property(schema)).append(" IS NOT NULL");

            case ILIKE, LIKE -> appendLikeQuery(query, attrValue, schema, cond, parameters);

            case IEQ, EQ -> appendEqualityQuery(query, attrValue, schema, cond, parameters);

            case GE -> appendComparisonQuery(query, attrValue, schema, cond, ">=", parameters);

            case GT -> appendComparisonQuery(query, attrValue, schema, cond, ">", parameters);

            case LE -> appendComparisonQuery(query, attrValue, schema, cond, "<=", parameters);

            case LT -> appendComparisonQuery(query, attrValue, schema, cond, "<", parameters);
        }
    }

    private void appendLikeQuery(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final PlainSchema schema,
            final AttrCond cond,
        ...
        boolean ignoreCase = cond.getType() == AttrCond.Type.ILIKE;
        String parameter = setParameter(parameters, regexValue(attrValue, cond));
        if (ignoreCase) {
            query.append("toLower (").append(property(schema)).append(") =~ toLower($").append(parameter).append(')');
        } else {
            query.append(property(schema)).append(" =~ $").append(parameter);
        }
    }

    private void appendEqualityQuery(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final PlainSchema schema,
            final AttrCond cond,
        ...
        boolean ignoreCase = schema.getType().isStringClass() && cond.getType() == AttrCond.Type.IEQ;
        String parameter = setParameter(parameters, plainValue(attrValue, cond));
        if (ignoreCase) {
            query.append("toLower (").append(property(schema)).append(")=toLower($").append(parameter).append(')');
        } else {
            query.append(property(schema)).append("=$").append(parameter);
    ...
    private void appendComparisonQuery(
            final TextStringBuilder query,
            final PlainAttrValue attrValue,
            final PlainSchema schema,
            final AttrCond cond,
        ...
        query.append(property(schema)).append(operator).append('$').append(setParameter(parameters, plainValue(attrValue, cond)));
    }

    private String plainValue(final PlainAttrValue attrValue, final AttrCond cond) {
        return Optional.ofNullable(attrValue.getDateValue()).
                map(DateTimeFormatter.ISO_OFFSET_DATE_TIME::format).
                orElseGet(cond::getExpression);
    }

    private String regexValue(final PlainAttrValue attrValue, final AttrCond cond) {
        return AnyRepoExt.escapeForLikeRegex(plainValue(attrValue, cond)).replace("%", ".*");
    }

    private String property(final PlainSchema schema) {
        return "n." + schema.getKey();
    }
